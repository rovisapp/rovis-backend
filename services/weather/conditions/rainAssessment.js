  ...
  calculatePrecipitationIntensity(params) {
    ...
    return Math.min(1.0, Math.max(0.0,
      calculateWeightedScores([
        {
          value: params.PRATE * 3600, // Convert to mm/hr
        ...
        {
          value: params.CPRAT * 3600,
          threshold: config.CONV_RATE.threshold,
          weight: config.CONV_RATE.weight
        }
      ])
    ));
}
      ...
      calculateWeightedScores([
        {
          ...
          // Special handling for VIS which needs inverse normalization
          // for eg, 
          // High visibility (e.g., 10,000 meters) = Good conditions = Should result in LOW severity score
          // Low visibility (e.g., 100 meters) = Poor conditions = Should result in HIGH severity score
          transform: (normalized) => 1 - normalized
        },
        {
          value: params.LCDC,
          threshold: config.LOW_CLOUD_COVER.threshold,
          weight: config.LOW_CLOUD_COVER.weight
        }
      ])
    ));
}
      ...
      calculateWeightedScores([
        {
          ...
          weight: config.RH.weight
        },
        {
          value: params.PWAT,
          threshold: config.PWAT.threshold,
          weight: config.PWAT.weight
        },
        {
          value: Math.abs(params.PRES_TREND),
          threshold: config.PRESSURE_TREND.threshold,
          weight: config.PRESSURE_TREND.weight
        }
      ])
    ));
}
